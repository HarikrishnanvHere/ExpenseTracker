...
function showPagination(currentPage,hasNextPage,nextPage,hasPreviousPage,previousPage) {
    ...
    if(!hasPreviousPage){
        button.innerHTML = `< ${currentPage} <button onclick="fetchExpenses(${nextPage})"> > </button>`;
    }
    else if(hasNextPage && hasPreviousPage){
        button.innerHTML= `<button onclick="fetchExpenses(${previousPage})"> <  </button> ${currentPage} <button onclick="fetchExpenses(${nextPage})"> > </button>`;
    }
    else if(!hasNextPage && hasPreviousPage){
        button.innerHTML= `<button onclick="fetchExpenses(${previousPage})"> <  </button> ${currentPage}`;
    }
}
